...
public class UserRepositoryImpl implements UserRepository {
    ...
    private final String FIND_BY_ID = """
            ...
            FROM ingame365.users u
                     LEFT JOIN ingame365.users_roles ur on u.id = ur.user_id
                     LEFT JOIN ingame365.users_tasks ut on u.id = ut.user_id
                     LEFT JOIN ingame365.tasks t on ut.task_id = t.id
            WHERE u.id = ?""";
    ...
    private final String CREATE = """
            INSERT INTO ingame365.users
            (name, username, password)
            VALUES (?, ?, ?)""";

    private final String INSERT_USER_ROLE = """
            INSERT INTO ingame365.users_roles
            (user_id, role)
            VALUES (?, ?)""";
    ...
    private final String IS_TASK_OWNER = """
            SELECT exists(
                   SELECT 1
                   FROM ingame365.users_tasks
                   WHERE user_id = ?
                   AND task_id = ?
            )""";

    private final String DELETE = """    
            DELETE FROM ingame365.users
            WHERE id = ?""";
    ...
    public Optional<User> findById(Long id) {
        try {
            ...
            //можно будет по списку ходить взад и вперед
